...
- **Root Causes Identified**:
  ...
  2. Missing `--close-on-child-death` flag in toggle-kitty-window.sh
  3. Dotfiles not deployed to runtime locations (~/.scripts/, ~/.config/)
- **Changes Made**:
...
- **Issue**: Alt+Shift+c (clipboard) not working after yesterday's changes
- **Root Causes**:
