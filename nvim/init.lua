...
-- Folding settings - disable folding globally
vim.opt.foldenable = false    -- Start with all folds open
vim.opt.foldmethod = "manual" -- Use manual folding (no auto-folding)
vim.opt.foldlevel = 99        -- Open all folds by default
vim.opt.foldlevelstart = 99   -- Always start with all folds open
...
-- Telescope keymaps (will be available after Telescope loads)
...
-- NOTE: Kulala keymaps defined in plugin's lazy.nvim 'keys' table for lazy loading

-- Database keymaps (vim-dadbod-ui)
...
-- NOTE: SQL-specific keymaps (<leader>De, <leader>Db, <leader>Ds, <leader>Dt) are set in FileType autocmd
-- NOTE: LSP keymaps (gd, gr, K, etc.) are set in LspAttach autocmd at bottom of file
...
require("lazy").setup({
  ...
  -- Telescope
  {
    ...
    config = function()
      ...
      -- Keymaps moved to consolidated KEYMAPS section above
    end,
  },
  ...
  -- REST client
  {
    ...
    opts = {
      -- UI configuration
      default_view = "body",                                        -- Always show response body expanded by default
      default_winbar_panes = { "body", "headers", "headers_body" }, -- Show body pane by default
      winbar = true,                                                -- Enable winbar for navigation
      -- Disable problematic kulala window keymaps to avoid cursor position errors
      kulala_keymaps = {
        ["Jump to response"] = false, -- Disable <CR> keymap that causes cursor errors
      },
    },
    config = function(_, opts)
      require("kulala").setup(opts)

      -- Auto-unfold (expand) response bodies
      vim.api.nvim_create_autocmd("FileType", {
        pattern = "kulala-response",
        callback = function()
          -- Disable folding entirely in kulala response buffers
          vim.opt_local.foldenable = false
          vim.opt_local.foldmethod = "manual"
        end,
      })

      -- Also unfold when opening any kulala UI buffer
  ...
  -- Database client (vim-dadbod)
  {
    ...
    config = function()
      -- Basic keymaps moved to consolidated KEYMAPS section above
      -- SQL file specific keymaps (kept here due to FileType autocmd context)
      vim.api.nvim_create_autocmd("FileType", {
        pattern = { "sql", "mysql", "plsql" },
        callback = function()
          -- Execute query and send to pspg
          local function execute_to_pspg(is_visual)
            vim.notify("execute_to_pspg called, is_visual=" .. tostring(is_visual), vim.log.levels.INFO)
            local query
            if is_visual then
          ...
          vim.keymap.set("n", "<leader>De", function() execute_to_pspg(false) end,
            { buffer = true, desc = "[D]atabase [E]xecute to pspg" })
          vim.keymap.set("v", "<leader>De", function() execute_to_pspg(true) end,
            { buffer = true, desc = "[D]atabase [E]xecute selection to pspg" })

          -- Execute query to buffer (original behavior)
          vim.keymap.set("n", "<leader>Db", "<Plug>(DBUI_ExecuteQuery)",
            { buffer = true, desc = "[D]atabase execute to [B]uffer" })
          vim.keymap.set("v", "<leader>Db", "<Plug>(DBUI_ExecuteQuery)",
            { buffer = true, desc = "[D]atabase execute to [B]uffer" })

          -- Save query to file
          vim.keymap.set("n", "<leader>Ds", "<Plug>(DBUI_SaveQuery)", { buffer = true, desc = "[D]atabase [S]ave query" })
          -- Toggle result layout (expanded view)
          vim.keymap.set("n", "<leader>Dt", "<Plug>(DBUI_ToggleResultLayout)",
            { buffer = true, desc = "[D]atabase [T]oggle result layout" })
        end,
      })
    end,
  },
