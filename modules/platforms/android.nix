{ pkgs, lib, ... }:
{
  ...
  environment.packages = with pkgs; [
    ...
    # Minimal dev stack matching nvim: Go / Rust / React(JS/TS) / Lua
    gcc
    gnumake
    ...
    openssh
    net-tools
    # mosh      # removed: not supported on nix-on-droid
...
if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
  ...
  echo "  Wi-Fi:    ssh -p 8022 nix-on-droid@''${DEVICE_IP}"
  echo "  USB:      adb forward tcp:8022 tcp:8022"
  echo "            ssh -p 8022 nix-on-droid@localhost"
  echo "  Tailscale: ssh -p 8022 nix-on-droid@<tailscale-ip>"
  echo "  (With key: add -i ~/.ssh/android_client_key)"
  echo "─────────────────────────────────────────────────────────────────"
  echo ""
  echo "  Auth: password or key-based"
  echo "  Set password: passwd"
  echo "  Client key:   $HOME/.ssh/android_client_key"
else
